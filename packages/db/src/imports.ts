...
export class CsvImportAccessError extends Error {}
...
export async function assertCsvImportTableAccess(
  db: Database,
  scope: ImportScope
): Promise<void> {
  const [table] = await db
    ...
    .limit(1);
  if (!table) throw new CsvImportAccessError('The table is not accessible.');
}
...
export async function createCsvImportJob(
  ...
  // Validate headers before a job becomes visible. Existing columns are
  // checked again by the worker when it freezes the final mapping.
  try {
    planCsvColumns(input.headers, []);
  } catch (error) {
    if (error instanceof CsvImportDefinitionError) {
      throw new CsvImportValidationError(error.message);
    }
    throw error;
  }
  await assertCsvImportTableAccess(db, input);
  const [job] = await db
    .insert(importJobs)
    .values({
  ...
  return toCsvImportSummary(job);
}

export async function stageCsvImportRows(
  db: Database,
  input: {
    importJobId: string;
  ...
  await db.transaction(async (tx) => {
    ...
    if (!job) throw new CsvImportAccessError('The import is not accessible.');
...
export async function queueCsvImport(
  ...
  return db.transaction(async (tx) => {
    ...
    const [queued] = await tx
      ...
      .where(eq(importJobs.id, job.job.id))
      .returning();
    if (!queued) throw new Error('The CSV import could not be queued.');
    ...
    return toCsvImportSummary(queued);
  });
}

export async function failCsvImportUpload(
  db: Database,
  input: {
    errorMessage: string;
    importJobId: string;
  ...
  await db.transaction(async (tx) => {
    ...
    if (failed) {
      await tx
        ...
        .where(eq(importStagedRows.importJobId, failed.id));
    }
  });
}
...
export async function listCsvImports(
  ...
  return db
    ...
    .limit(10)
    .then((records) => records.map(({ job }) => toCsvImportSummary(job)));
}
...
export async function prepareCsvImport(
  ...
  return db.transaction(async (tx) => {
    const [job] = await tx
      ...
      .limit(1)
      .for('update');
    if (!job) throw new CsvImportAccessError('The import job does not exist.');
...
export async function applyCsvImportBatch(
  db: Database,
  input: { importJobId: string; tableId: string; workspaceId: string },
  batchSize = 250
  ...
  return db.transaction(async (tx) => {
    const [job] = await tx
      ...
      .for('update');
    if (!job)
      throw new CsvImportAccessError('The running import was not found.');
    ...
    for (const rowDefinition of rowValues) {
      const rowId = rowIdByPosition.get(rowDefinition.position)!;
...
export async function setCsvImportWorkerFailure(
  db: Database,
  input: {
    errorMessage: string;
    importJobId: string;
...
function validateFilename(filename: string): string {
  ...
  if (!normalized || normalized.length > 255 || /\p{Cc}/u.test(normalized)) {
    throw new CsvImportValidationError('The CSV filename is invalid.');
  }
  return normalized;
}
